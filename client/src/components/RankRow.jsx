  ...
  return (
    ...
    >
      <div className="text-center text-sm flex items-center justify-center gap-1" style={{ fontFamily: t.titleFont }}>
        {rankStyle.icon ? (
          <motion.span animate={{ rotate: [0, -5, 5, 0] }} transition={{ repeat: Infinity, repeatDelay: 3, duration: 0.6 }} className="text-lg cursor-default">
            {rankStyle.icon}
          </motion.span>
        ) : (
        ...
        {rankChanged && (
          <motion.span initial={{ y: prevRow.rank < row.rank ? -10 : 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }}
            className="text-[10px] ml-0.5" style={{ color: prevRow.rank < row.rank ? "#ff6666" : t.accent }}>
      ...
      <motion.div layout className="flex items-center font-bold text-sm truncate" style={{ color: t.text }}>
        {isTop3 && (
          <motion.div layout className="w-1.5 h-4 rounded-full mr-1.5"
            style={{ background: row.rank === 1 ? "#ffd700" : row.rank === 2 ? "#c0c0c0" : "#cd7f32" }} />
        )}
        {row.name}
      </motion.div>

      <div className="text-center text-xs flex items-center justify-center" style={{ color: t.muted }}>
        {row.grade}{row.class}
      </div>
      ...
      <div className="text-center flex items-center justify-center">
        <motion.span
          ...
          animate={{ scale: 1, color: row.completion_rate >= 90 ? "#39ff14" : row.completion_rate >= 70 ? "#ffff00" : t.text }}
          transition={{ type: "spring", stiffness: 200 }}
          className="font-mono text-sm"
        >{row.completion_rate}%</motion.span>
      </div>
      ...
      <div className="text-center flex items-center justify-center">
        <motion.span
          ...
          transition={{ type: "spring", stiffness: 200 }}
          className="font-mono text-sm"
          style={{ color: row.max_combo >= 500 ? t.secondary : t.text }}
        >{row.max_combo}</motion.span>
      </div>
      ...
      <div className="text-center flex items-center justify-center">
        <motion.span
          ...
          className="font-mono text-sm"
          style={{ color: row.perfect_count >= 300 ? t.primary : t.text }}
        >{row.perfect_count}</motion.span>
      </div>
    </motion.div>
  );
}
